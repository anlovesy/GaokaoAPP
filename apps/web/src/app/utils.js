...
export function createPlanReadyMessage(mode = DEFAULT_ADVISOR_MODE, chatEnabled = true) {
  ...
  if (mode === "xuefeng") {
    return "方案已经出来了。接下来别急着高兴，先把最关键的几件事聊透：为什么这么排、哪些专业组看着稳其实最容易翻车、如果只留广东要不要降层次、哪些位置最容易因为调剂和组内冷热差出问题。";
  }
...
export function resolveInitialScreen(hasAuthToken) {
  if (typeof window === "undefined") {
    return hasAuthToken ? SCREEN_NAVIGATION : SCREEN_LANDING;
  }

  const storedEntryMode = window.localStorage.getItem(ENTRY_MODE_STORAGE_KEY);
  return resolveScreenFromPath(window.location.pathname, {
    hasAuthToken,
    hasGuestAccess: storedEntryMode === "guest"
  });
}

export function resolveScreenFromPath(pathname, accessState) {
  const access =
    typeof accessState === "boolean"
      ? { hasAuthToken: accessState, hasGuestAccess: accessState }
      : accessState || { hasAuthToken: false, hasGuestAccess: false };
  const hasAuthToken = Boolean(access.hasAuthToken);
  const hasGuestAccess = Boolean(access.hasGuestAccess);
  const hasAppAccess = hasAuthToken || hasGuestAccess;

  if (pathname === SCREEN_PATH_MAP[SCREEN_AUTH]) {
    return SCREEN_AUTH;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_NAVIGATION]) {
    return hasAppAccess ? SCREEN_NAVIGATION : SCREEN_LANDING;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_ADVISOR]) {
    return hasAppAccess ? SCREEN_ADVISOR : SCREEN_LANDING;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_HISTORY]) {
    return hasAppAccess ? SCREEN_HISTORY : SCREEN_LANDING;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_ACCOUNT]) {
    return hasAuthToken ? SCREEN_ACCOUNT : SCREEN_LANDING;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_ADMIN_USERS]) {
    return hasAuthToken ? SCREEN_ADMIN_USERS : SCREEN_LANDING;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_UNIVERSITY]) {
    return hasAppAccess ? SCREEN_UNIVERSITY : SCREEN_LANDING;
  }

  if (pathname === SCREEN_PATH_MAP[SCREEN_WORKSPACE]) {
    return hasAppAccess ? SCREEN_WORKSPACE : SCREEN_LANDING;
  }
...
export function buildAdvisorContextHighlights({ formState, profileHighlights, planStats, result }) {
  const summary = [
    ...
    {
      label: "分数 / 位次",
      value: `${formState.score || "--"} 分 / ${formState.rank || "--"} 位`
    },
    {
      label: "选科",
