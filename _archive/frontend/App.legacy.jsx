...
import { getUniversityProfile, getUniversityResourceLinks, universityProfiles } from "./universityProfiles.js";
...
function App() {
  ...
  const [selectedUniversityDetail, setSelectedUniversityDetail] = useState(() => readStoredUniversityDetail());
...
function UniversityScreen({ university, onBack }) {
  return (
...
function UniversityDetailPanel({ university, onOpenStandalone = null, isStandalone = false }) {
  ...
  const focusMajors = Array.from(
    new Set((profile.keyMajors?.length ? profile.keyMajors : university.schools.map((school) => school.major)).filter(Boolean))
  ).slice(0, 6);
  ...
  return (
    <div className="auth-shell">
      <div className="auth-wall" aria-hidden="true">
        {AUTH_WALL_ROWS.map((row, rowIndex) => (
...
function buildAuthWallRows(profiles) {
...
function buildUniversityGallery(result) {
...
function collectPlanSchools(result) {
  if (!result) {
    return [];
  }
...
function resolveSchoolRankValue(school) {
  return school?.minRank || school?.threshold || "--";
}

function formatTuitionText(value) {
  if (!value) {
    return "待补充";
  }
...
function readStoredUniversityDetail() {
  if (typeof window === "undefined") {
    return null;
  }
...
function writeStoredUniversityDetail(university) {
  if (typeof window === "undefined") {
    return;
  }
...
function clearStoredUniversityDetail() {
  if (typeof window === "undefined") {
    return;
  }
